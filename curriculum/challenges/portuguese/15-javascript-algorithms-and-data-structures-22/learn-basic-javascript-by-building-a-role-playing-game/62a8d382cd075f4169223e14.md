...
Observe que você já tem uma variável `currentWeapon` em outro lugar no código. Since this new `currentWeapon` variable will be inside an `if` statement, it will be scoped only to that block of code.
...
Use a palavra-chave `let` para criar uma variável chamada `currentWeapon`. Ainda não atribua um valor.

# --hints--

Você deve usar `let` para declarar a variável `currentWeapon`.
