...
Usando a mesma sintaxe, defina as propriedades `onclick` de `button2` e `button3` como `goCave` e `fightDragon`, respectivamente.

Assim que você tiver feito isso, abra o console e tente clicar nos botões do projeto.
...
Você deve usar a notação do ponto para acessar a propriedade `onclick` do `button2`.

```js
assert.match(code, /button2\.onclick/);
```

Você não deve usar `let` ou `const` para atribuir `button2.onclick`.
...
Você deve usar a notação do ponto para acessar a propriedade `onclick` do `button3`.

```js
assert.match(code, /button3\.onclick/);
```

Você não deve usar `let` ou `const` para atribuir `button3.onclick`.
