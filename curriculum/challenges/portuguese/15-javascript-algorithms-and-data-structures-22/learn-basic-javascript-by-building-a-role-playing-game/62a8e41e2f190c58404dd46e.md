...
Você deve usar notação do ponto para acessar a propriedade `innerText` do `text` em uma nova linha.
...
Você deve usar a atribuição composta com `text.innerText`.

```js
assert.match(attack.toString(), /text\.innerText\s*\+=/);
```

You should add the string `" You attack it with your "` to the `text.innerText` value. Lembre-se de que o espaçamento é importante.

```js
assert.match(attack.toString(), /text\.innerText\s*\+=\s*('|") You attack it with your \1/);
```

Você deve usar o operador de concatenação para adicionar a arma atual à string. Você pode obter a arma atual com `weapons[currentWeapon].name`.
