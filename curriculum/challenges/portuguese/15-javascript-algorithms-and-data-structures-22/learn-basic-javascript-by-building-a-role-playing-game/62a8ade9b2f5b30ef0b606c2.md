...
Adicione uma propriedade final ao objeto chamada `text`. Give this property the same string value as the one assigned to `text.innerText` in the `goTown` function.

# --hints--

O primeiro valor de `locations` deve ter uma propriedade `text`.

```js
assert.isDefined(locations[0]["text"]);
```

A propriedade `text` deve ser uma string.
