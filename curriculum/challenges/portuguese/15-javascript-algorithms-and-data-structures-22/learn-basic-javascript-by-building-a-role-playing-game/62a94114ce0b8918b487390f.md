...
Adicione outro objeto ao array `locations`. Everything should be the same as the `lose` object, except the `name` should be `"win"` and the `text` should be `"You defeat the dragon! YOU WIN THE GAME! &#x1F389;"`.

# --hints--

Você deve ter sete valores no array `locations`.

```js
assert.lengthOf(locations, 7);
```

O sétimo valor de `locations` deve ser um objeto.
...
O sétimo valor de `locations` deve ter um array `button functions` com três variáveis `restart`.
