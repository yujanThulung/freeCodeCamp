...
Na função `defeatMonster`, defina `gold` como `gold` mais o nível do monstro vezes `6.7`. Você pode obter o nível do monstro com a propriedade `level`.

Aqui está um exemplo de como definir `num` com `num` mais `5` * `8`: `num += 5 * 8`. Use `Math.floor()` para arredondar o resultado para baixo.

# --hints--

Você deve usar a atribuição composta para modificar `gold`.

```js
assert.match(defeatMonster.toString(), /gold\s*\+=/);
```

Você deve usar `Math.floor()` para arredondar o resultado do nível do monstro vezes `6.7`.
