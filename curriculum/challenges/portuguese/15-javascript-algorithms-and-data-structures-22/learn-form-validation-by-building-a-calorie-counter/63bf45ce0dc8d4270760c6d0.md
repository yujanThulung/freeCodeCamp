...
Within your loop, you need to check if the character in `strArray` at index `i` is not a `+`, `-`, or a space. Se não for, faça o `push` do caractere (em outras palavras, insira-o) em `cleanStrArray`.
