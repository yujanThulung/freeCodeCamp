...
Crie uma `div` e dê a ela uma `class` `controls`. Coloque um elemento `span` dentro do elemento `div`.

# --hints--

Você deve criar um elemento `div` no `form`.

```js
assert.exists(document.querySelector('form > div'));
```

O elemento `div` deve vir depois do elemento `fieldset`.
...
O elemento `div` deve ter o atributo `class` definido como `controls`.

```js
assert.equal(document.querySelector('form > div')?.className, "controls");
```

Você deve criar um elemento `span` dentro do elemento `div`.
