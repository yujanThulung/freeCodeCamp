...
Declare uma variável `surplusOrDeficit`. Then use a ternary operator to set `surplusOrDeficit` to the string `Surplus` or `Deficit` depending on whether `remainingCalories` is less than `0`. If it is less than `0`, then `surplusOrDeficit` should be `Surplus`. Caso contrário, ele deverá ser `Deficit`.

# --hints--

A função `calculateCalories` deve ter uma variável `surplusOrDeficit`.
...
Você deve atribuir o resultado do operador ternário a `surplusOrDeficit`.
