...
Na nova função, declare uma variável `calories` e atribua a ela o valor `0`. Use `let` para declará-la, já que você vai reatribuir valores a ela mais tarde.

# --hints--

Você deve declarar uma variável `calories`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=/);
```

Você deve atribuir à variável `calories` o valor `0`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=\s*0/);
```

Você deve declarar a variável `calories` com `let`.
