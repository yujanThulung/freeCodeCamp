...
Оголосіть змінну під назвою `budgetNumberInput`.

```js
assert.isDefined(budgetNumberInput);
```

Ви повинні використати `document.getElementById()`, щоб отримати елемент `#budget`.

```js
assert.match(code, /document\.getElementById\(\s*('|")budget\1\s*\)/g);
```

Ви повинні зберегти елемент `#budget` у змінній під назвою `budgetNumberInput`.

```js
assert.equal(budgetNumberInput, document.getElementById('budget'));
```

Оголосіть змінну під назвою `entryDropdown`.

```js
assert.isDefined(entryDropdown);
```

Ви повинні використати `document.getElementById()`, щоб отримати елемент `#entry-dropdown`.

```js
assert.match(code, /document\.getElementById\(\s*('|")entry-dropdown\1\s*\)/g);
```

Ви повинні зберегти елемент `#entry-dropdown` у змінній під назвою `entryDropdown`.
