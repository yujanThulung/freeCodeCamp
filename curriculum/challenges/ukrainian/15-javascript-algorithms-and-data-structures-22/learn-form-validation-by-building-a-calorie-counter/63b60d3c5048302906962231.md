...
Розпочніть з отримання елементу `form` (використовуючи `id`) та збережіть його у змінній під назвою `calorieCounter`.

# --hints--

Ви повинні створити змінну під назвою `calorieCounter`.

```js
assert.isDefined(calorieCounter);
```

Ви повинні використати `document.getElementById()`, щоб отримати елемент `#calorie-counter`.

```js
assert.match(code, /document\.getElementById\(\s*('|")calorie-counter\1\s*\)/g);
```

Ви повинні зберегти елемент `#calorie-counter` у змінній під назвою `calorieCounter`.
