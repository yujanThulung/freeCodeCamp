...
Denke daran, dass `return` die Ausführung einer Funktion beendet. After your `if` block, you need to handle the logic for when the input is valid. Because your `if` statement returns a value, you do not need an `else` statement.
