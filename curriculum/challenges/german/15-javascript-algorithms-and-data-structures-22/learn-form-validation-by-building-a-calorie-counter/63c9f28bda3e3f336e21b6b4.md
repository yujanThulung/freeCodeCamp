...
Um diese Funktion abzuschließen, musst du die `hide`-Klasse im `output`-Element wiederherstellen. The `classList` property has an `.add()` method which is the opposite of the `.remove()` method. It accepts a string representing the class to add to the element.
