...
Der `list`-Parameter wird das Ergebnis eines Query-Selektors sein, der eine `NodeList` zurückgeben wird. A `NodeList` is a list of elements like an array. It contains the elements that match the query selector. You will need to loop through these elements in the list.
