...
Setze in deiner `defeatMonster`-Funktion `gold` gleich `gold` plus das Level des Monsters mal `6.7`. Du kannst das Level des Monsters mit der `level`-Eigenschaft erhalten.

Hier ist ein Beispiel für das Setzen von `num` auf `num` plus `5` * `8`: `num += 5 * 8`. Verwende `Math.floor()`, um das Ergebnis abzurunden.

# --hints--

Du solltest die zusammengesetzte Zuweisung verwenden, um `gold` zu ändern.

```js
assert.match(defeatMonster.toString(), /gold\s*\+=/);
```

Du solltest `Math.floor()` verwenden, um das Resultat des Monster-Levels mal `6.7` zu runden.
