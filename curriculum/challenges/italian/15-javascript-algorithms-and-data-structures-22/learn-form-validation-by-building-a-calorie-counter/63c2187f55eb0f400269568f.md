...
Infine, su una nuova riga dopo il secondo `label`, crea un altro elemento `input`. Dagli un attributo `type` impostato su `number`, un attributo `min` impostato a `0` (per non consentire di aggiungere calorie negative), un attributo `placeholder` impostato su `Calories` e un attributo `id` corrispondente all'attributo `for` del secondo elemento `label`.

# --hints--

Dovresti avere due elementi `input` in `HTMLString`.
...
Il nuovo elemento `input` dovrebbe essere su una nuova riga.
...
Il nuovo elemento `input` dovrebbe trovarsi dopo il secondo elemento `label`.
...
Il nuovo elemento `input` dovrebbe avere un attributo `type` impostato su `number`.
...
L'elemento `input` dovrebbe avere un attributo `placeholder` impostato su `Calories`.
