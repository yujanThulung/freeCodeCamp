...
Nel testo dello `span` racchiudi il riferimento a `remainingCalories` in `Math.abs()` per assicurarti che il valore sia positivo.

# --hints--

Dovresti racchiudere la variabile `remainingCalories` in `Math.abs()`.
...
Lo `span` dovrebbe avere il testo `${Math.abs(remainingCalories)} Calorie ${surplusOrDeficit}`.
