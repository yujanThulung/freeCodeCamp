...
Nella nuova funzione, dichiara una variabile `calories` e assegnale il valore `0`. Usa `let` per dichiararla, dato che la riassegnerai più tardi.

# --hints--

Dovresti dichiarare una variabile `calories`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=/);
```

Alla variabile `calories` dovrebbe essere assegnato il valore di `0`.

```js
assert.match(getCaloriesFromInputs.toString(), /calories\s*=\s*0/);
```

Dovresti dichiarare una variabile `calories` con `let`.
