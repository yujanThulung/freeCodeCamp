...
Nel progetto del gioco di ruolo hai imparato come definire il comportamento di un pulsante modificando la sua proprietà `onclick`. Puoi anche modificare il comportamento di un elemento aggiungendo un event listener (ascoltatore di eventi).
...
Nota che non dovresti *chiamare* `addEntry`, ma passare direttamente la variabile (o <dfn>riferimento della funzione</dfn>).

# --hints--

Dovresti chiamare il metodo `.addEventListener()` su `addEntryButton`.

```js
assert.match(code, /addEntryButton\.addEventListener\(/);
```

Dovresti passare `click` come primo argomento al metodo `.addEventListener()`.

```js
assert.match(code, /addEntryButton\.addEventListener\(\s*('|")click\1\s*/);
```

Dovresti passare `addEntry` come secondo argomento al metodo `.addEventListener()`.
