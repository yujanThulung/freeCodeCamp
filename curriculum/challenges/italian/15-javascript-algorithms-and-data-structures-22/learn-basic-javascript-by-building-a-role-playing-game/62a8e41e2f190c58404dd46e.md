...
Dovresti usare la dot notation per accedere alla proprietà `innerText` di `text` su una nuova riga.
...
Dovresti usare l'assegnazione composta con `text.innerText`.

```js
assert.match(attack.toString(), /text\.innerText\s*\+=/);
```

You should add the string `" You attack it with your "` to the `text.innerText` value. Ricorda che la spaziatura è importante.

```js
assert.match(attack.toString(), /text\.innerText\s*\+=\s*('|") You attack it with your \1/);
```

Dovresti usare l'operatore di concatenazione per aggiungere l'arma attuale alla stringa. Puoi ottenere l'arma attuale con `weapons[currentWeapon].name`.
