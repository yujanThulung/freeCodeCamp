...
Nella funzione `defeatMonster`, imposta `gold` uguale a `gold` più il livello del mostro per `6.7`. Puoi ottenere il livello del mostro con la proprietà `level`.

Ecco un esempio dell'impostazione di `num` su `num` più `5` * `8`: `num += 5 * 8`. Usa `Math.floor()` per arrotondare il risultato per difetto.

# --hints--

Dovresti usare l'assegnazione composta per modificare `gold`.

```js
assert.match(defeatMonster.toString(), /gold\s*\+=/);
```

Dovresti usare `Math.floor()` per arrotondare il risultato del livello del mostro moltiplicato per `6.7`.
